...
function card(lab){return `<article class="card"><div class="score">${fmt(lab.total_score||score(lab))}</div><h3>${lab.name}</h3><p><strong>${lab.institution||'Institution not specified'}</strong></p><p>${lab.city?lab.city+', ':''}${lab.country}</p><div><span class="pill ${tierClass(lab.validation_status)}">${lab.validation_status}</span><span class="pill">${lab.confidence_level} confidence</span><span class="pill">${primaryDomain(lab)}</span></div><p>${(lab.summary||'').slice(0,180)}${(lab.summary||'').length>180?'…':''}</p><div class="bar"><span style="width:${Math.min(100,lab.evidence_coverage||0)}%"></span></div><p class="muted">Evidence coverage: ${lab.evidence_coverage||0}% · Missing criteria: ${lab.missing_criteria_count||0}</p><div class="card-actions"><a class="button" href="${labUrl(lab.id)}">Open profile</a><a class="button" href="compare.html?add=${encodeURIComponent(lab.id)}">Compare</a></div></article>`}
